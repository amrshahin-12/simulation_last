...
private:
	...
	void GetRandomWaitingList(char& listType) {
		...
		cout << "Randomly selected waiting list: " << listType << endl;
	}
...
public:
	...
	void SimulationV2() {
		...
		for (int i = 0; i < 100; i++) // 100 timesteps
		{
			...
			if (X < 10) {
				...
				if (!Early_Patients.isEmpty()) {
					int pri;
					if (Early_Patients.dequeue(p, pri) && p) {
						char listType;
						GetRandomWaitingList(listType);
						switch (listType) {
						case 'E': E_Waiting.enqueue(p); break;
						case 'U': U_Waiting.enqueue(p); break;
						case 'X': X_Waiting.enqueue(p); break;
						}
						cout << "x was: " << X << " so move from early list to random waiting list\n";
					}
				}
			}
				else if (X < 20) {
					...
					if (!Late_Patients.isEmpty()) {
						int pri;
						if (Late_Patients.dequeue(p, pri) && p) {
							char listType;
							GetRandomWaitingList(listType);
							switch (listType) {
							case 'E': E_Waiting.InsertSorted(p); break;
							case 'U': U_Waiting.InsertSorted(p); break;
							case 'X': X_Waiting.InsertSorted(p); break;
							}
							cout << "x was: " << X << " so move from late list to random waiting list\n";
						}
					}
				}
				else if (X < 40) {
					...
					GetRandomWaitingList(listType);
					for (int i = 0; i < 2; ++i) {
						Patient* tempP = nullptr;
						//bool dequeued = false;
						switch (listType) {
						case 'E':  E_Waiting.dequeue(tempP); break;
						case 'U':  U_Waiting.dequeue(tempP); break;
						case 'X':  X_Waiting.dequeue(tempP); break;
						}
						if ( tempP) {
							In_Treatment.enqueue(tempP, tempP->PT);
						}
					}
					cout << "x was: " << X << " so move 2 p from random list to in treatment\n";
					...
					if (!In_Treatment.isEmpty()) {
						int pri;
						if (In_Treatment.dequeue(p, pri) && p) {
							char listType;
							GetRandomWaitingList(listType);
							switch (listType) {
							case 'E': E_Waiting.InsertSorted(p); break;
							...
							cout << "x was: " << X << " so move from InTreatment list to random waiting list\n";
						}
					}
				}
				else if (X < 60) {
					...
					if (!In_Treatment.isEmpty()) {
						...
						if (In_Treatment.dequeue(temp, pri) && temp) {
							Finished_Patients.push(temp);
							cout << "x was: " << X << " so move from InTreatment to finished list\n";
						}
					}
					...
					cout << "x was: " << X << " so move cancelled patients from X_Waiting to Finished\n";
					Patient* Cpatient = X_Waiting.GuaranteedCancel();
					if (Cpatient) {
					...
					cout << "x was: " << X << " reschedule a patient from Early_Patients\n";
